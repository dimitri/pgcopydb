...
The ``pgcopydb restore pre-data`` command implements the creation of SQL
objects in the target database, the second step of a full database
migration.
...
::
   ...
   $ PGCOPYDB_DROP_IF_EXISTS=on pgcopydb restore pre-data --source /tmp/target/ --target "port=54314 dbname=demo"
