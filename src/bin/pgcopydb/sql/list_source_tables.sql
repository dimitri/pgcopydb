WITH filters AS (
    SELECT
        ...
        $4::text[] AS incl_er_nsp,  $5::text[] AS incl_er_rel_re,
        -- include-only table: regex nsp + exact rel
        $6::text[] AS incl_re_nsp_re, $7::text[] AS incl_re_rel,
        -- include-only table: regex nsp + regex rel
        $8::text[] AS incl_rr_nsp_re, $9::text[] AS incl_rr_rel_re,
        -- exclude table: exact nsp + exact rel
        $10::text[] AS excl_ee_nsp, $11::text[] AS excl_ee_rel,
        -- exclude table: exact nsp + regex rel
        $12::text[] AS excl_er_nsp, $13::text[] AS excl_er_rel_re,
        -- exclude table: regex nsp + exact rel
        $14::text[] AS excl_re_nsp_re, $15::text[] AS excl_re_rel,
        -- exclude table: regex nsp + regex rel
        $16::text[] AS excl_rr_nsp_re, $17::text[] AS excl_rr_rel_re,
        -- exclude-data: exact nsp + exact rel
        $18::text[] AS xdat_ee_nsp, $19::text[] AS xdat_ee_rel,
        -- exclude-data: exact nsp + regex rel
        $20::text[] AS xdat_er_nsp, $21::text[] AS xdat_er_rel_re,
        -- exclude-data: regex nsp + exact rel
        $22::text[] AS xdat_re_nsp_re, $23::text[] AS xdat_re_rel,
        -- exclude-data: regex nsp + regex rel
        $24::text[] AS xdat_rr_nsp_re, $25::text[] AS xdat_rr_rel_re
)
SELECT c.oid,
       ...
       (
        ...
        OR EXISTS (SELECT 1 FROM unnest(f.xdat_er_nsp, f.xdat_er_rel_re) AS t(nsp, rel_re)
                   WHERE n.nspname = t.nsp AND c.relname ~ t.rel_re)
        OR EXISTS (SELECT 1 FROM unnest(f.xdat_re_nsp_re, f.xdat_re_rel) AS t(nsp_re, rel)
                   WHERE n.nspname ~ t.nsp_re AND c.relname = t.rel)
        OR EXISTS (SELECT 1 FROM unnest(f.xdat_rr_nsp_re, f.xdat_rr_rel_re) AS t(nsp_re, rel_re)
...
-- find a copy partition key candidate
 ...
 WHERE c.relkind IN ('r', 'm') AND c.relpersistence IN ('p', 'u')
   ...
   AND (
       ...
       AND f.incl_re_nsp_re IS NULL AND f.incl_rr_nsp_re IS NULL
